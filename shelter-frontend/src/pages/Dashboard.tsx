...
export default function Dashboard() {
  ...
  // CANLI BACKEND ADRESİ (Sonunda / işareti olmasın)
  ...
  const getImageUrl = (url) => {
    ...
    if (url.includes("uploads")) {
      // URL'i parçala ve dosya adını al
      const parcalar = url.split("uploads"); 
      const dosyaAdi = parcalar[parcalar.length - 1]; // Örn: /kedi.jpg veya \kedi.jpg
      
      // Temiz dosya adını oluştur (slat/ters slash temizliği)
  ...
  // ... (Diğer fonksiyonlar aynı kalıyor: handleSave, handleEdit vb.)
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-8 overflow-y-auto">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {hayvanlar.map((hayvan) => (
                      <tr key={hayvan.id} className="hover:bg-gray-50">
                        {/* --- KESİN ÇÖZÜM İÇİN DÜZELTİLEN KISIM --- */}
          ...
          {/* Diğer tablolar aynı kalacak, sadece üst kısmı güncellemen yeterli ama tamamını attım. */}
        </main>
      </div>
      {/* Modallar buraya gelecek (kodun kalanını koru) */}

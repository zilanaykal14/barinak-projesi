...
export default function Dashboard() {
  ...
  // Tür tanımlamaları (TypeScript için)
  ...
  // Listelerin türlerini any[] olarak belirledik ki hata vermesin
  ...
  // Form verisi için başlangıç değerleri
  const [formData, setFormData] = useState({
    ...
    irkId: "", 
    cipNo: "", 
    secilenAsilar: [] as string[] // String listesi olduğunu belirttik
  });
  ...
  // --- RESİM URL DÜZELTME FONKSİYONU (TypeScript) ---
  const getImageUrl = (url: string | null | undefined) => {
    if (!url) return "https://placehold.co/100";
    
    // Eğer resim URL'si zaten düzgün bir http linkiyse ve localhost DEĞİLSE dokunma
    if (url.startsWith("http") && !url.includes("localhost") && !url.includes("127.0.0.1")) {
      return url;
    }

    // Eğer linkte 'uploads' geçiyorsa (localhost olsa bile) temizle ve Render adresini koy
    ...
    // Hiçbiri değilse direkt API URL ekle
    return `${API_URL}${url}`;
  };
  // ---------------------------------------------------
  ...
  const handleIhbarGonder = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post(`${API_URL}/bildirim`, {
        tip: 'ihbar', mesaj: ihbarMesaj, gonderenAd: user.fullName, durum: 'Bekliyor'
      });
      alert("İhbar iletildi!");
      setIsIhbarOpen(false); setIhbarMesaj(""); fetchData(user);
    } catch (error) { alert("Hata!"); }
  };

  const handleSahiplenmeIstegi = async (hayvan: any) => {
    if (window.confirm(`${hayvan.ad} için istek gönderilsin mi?`)) {
      try {
        await axios.post(`${API_URL}/bildirim`, {
          tip: 'sahiplenme', mesaj: `${hayvan.ad} isimli hayvana talibim (ID: ${hayvan.id}).`, gonderenAd: user.fullName, hayvanId: hayvan.id, durum: 'Bekliyor'
        });
  ...
  const handleBildirimSil = async (id: number) => {
    if (window.confirm("Bildirimi silmek istiyor musunuz?")) {
      await axios.delete(`${API_URL}/bildirim/${id}`);
      fetchData(user);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const paket: any = {
        ...
        resimUrl: finalResimUrl || "https://placehold.co/200", irk: { id: parseInt(formData.irkId) }, asilar: formData.secilenAsilar.map(id => ({ id: parseInt(id) }))
      };
  ...
  const handleEdit = (hayvan: any) => {
    ...
    setSelectedFile(null); setIsModalOpen(true);
  };

  const handleDelete = async (id: number) => { if (window.confirm("Silinsin mi?")) { await axios.delete(`${API_URL}/hayvan/${id}`); setHayvanlar(hayvanlar.filter((h) => h.id !== id)); } };
  ...
  const handleCheckboxChange = (id: string) => { const s = formData.secilenAsilar; if(s.includes(id)) setFormData({...formData, secilenAsilar: s.filter(x=>x!==id)}); else setFormData({...formData, secilenAsilar: [...s, id]}); };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-8 overflow-y-auto">
          ...
          {/* ... Diğer sekmeler (Users, Bildirimler vb.) aynı kalır ... */}
